...
function buyAirtime() {
  ...
  let amount = Number(document.getElementById("airtimeAmount").value);
...
function confirmTopUp() {
    ...
    alert(`Processing top-up of ₦${Number(amount).toLocaleString()} ...\n\n(You would normally be redirected to payment gateway now)`);
    // Here you would usually integrate Flutterwave / Paystack inline / redirect
}

// Initialize top-up amount display
document.addEventListener("DOMContentLoaded", () => {
    updateAmount();
});

// ────────────────────────────────────────────────
// Crypto network accordion
// ────────────────────────────────────────────────

function showNetwork(network) {
    const networks = ["btc", "eth", "bnb", "sol", "trx", "xrp"];
    
    networks.forEach(n => {
        const details = document.getElementById(n + "Details");
        if (details) {
            details.style.display = (n === network) ? "block" : "none";
        }
    });
}
